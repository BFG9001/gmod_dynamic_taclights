...
local function DoThirdPersonTacLights()
	for k, ply in pairs(player.GetAll()) do
		if (ply ~= LocalPlayer()) and ply:GetNWBool("BFG_IsCustomFlashlightOn", true) then
			...
			local brightness = math.Clamp(BASE_BRIGHTNESS * (1/handTrace.Fraction), .5, BASE_BRIGHTNESS)
			local size = math.Clamp( 150 * (handTrace.Fraction), 50, 150 )
...
local function LightSprite(ply)
	if not (v == LocalPlayer()) and ply:GetNWBool("BFG_IsCustomFlashlightOn", false) then
			...
			render.DrawSprite(muzzlePos or handPos or ply:GetShootPos(), 10, 10, Color( 255,255,255 ))
	end
end
